...
export class AquaboxInstance extends Serializable {
    ...
    public localAvailable: boolean = false
    public cloudAvailable: boolean = false
    ...
    public constructor(public service: AquaBoxService,
        ...
        this.updateConsumer.setSenderFilter(this.configuration.serial);
        this.updateConsumer.setEventHandler((event: UpdateEvent) => {
            if (event.Properties.hasOwnProperty("cloudAvailable"))
                this.cloudAvailable = event.Properties["cloudAvailable"];
            if (event.Properties.hasOwnProperty("localAvailable"))
                this.localAvailable = event.Properties["localAvailable"];
            this.status.available = this.localAvailable || this.cloudAvailable;
        });
    ...
    public connected(): ConnectionMethods
    {
        return this.localAvailable
                ? ( this.cloudAvailable ? ConnectionMethods.Both : ConnectionMethods.LocalOnly )
                : ConnectionMethods.Disconnected;
    }
